...
def run_experiment(hparams, writer):
    ...
    if hparams.use_weighted_squared_error:
        ...
        G_w = None
        G_optimizer = optim.Adam(
            list(G.parameters()), lr=hparams.g_lr, betas=(beta1, beta2))
    ...
    if hparams.resume_training:
        ...
        G.load_state_dict(torch.load(os.path.join(experiment_path, 'G.hdf5')))
        if G_w is not None:
    ...
    for i_epoch in range(hparams.max_epochs):
        print("training step ...")
        result = fit(G, G_w, Ds, Drf, G_optimizer, Ds_optimizer, Drf_optimizer, trainloader, hparams, writer)
        print("validation step ...")
        (val_secret_acc, val_utility_acc), val_result = validate(G, G_w, Ds_fix, Du_fix, validloader, hparams)
        ...
        names = ['real images', 'fake_images']
        for i, name in enumerate(names):
            grid = torchvision.utils.make_grid(val_result[i])
            writer.add_image('valid/' + name, grid, 0)

        if hparams.use_weighted_squared_error:
            grid = torchvision.utils.make_grid(val_result[2])
            writer.add_image("valid/weights", grid, 0)

        # save stuff
        save_models(G, G_w, Ds, Drf, experiment_path) #, i_epoch)

def save_models(G, G_w, Ds, Drf, experiment_path): #, epoch):
    torch.save(G.state_dict(), os.path.join(experiment_path, "G.hdf5"))
    if not G_w is None:
...
def fit(G, G_w, Ds, Drf, G_optimizer, Ds_optimizer, Drf_optimizer, trainloader, hparams, writer):
    ...
    for i_batch, batch in tqdm.tqdm(enumerate(trainloader, 0)):
        ...
        # train generators
        if not Drf is None:
            z1n = torch.randn(len(images), hparams.noise_dim//2).to(hparams.device)
            fake_secret = torch.randint(0, 2, (len(images), 1)).float().to(hparams.device)
            z1l = fake_secret.repeat(1, hparams.noise_dim//2).to(hparams.device)
            z1 = torch.cat((z1n, z1l), dim=1)

        cen_images = G(images, z1)

        if i_batch % hparams.nb_discriminator_steps == 0:
            secret_pred_1 = Ds(cen_images)

            if not Drf is None:
                fake_secret_pred = Drf(cen_images)
                g_fake_secret_loss = secret_loss_function(fake_secret_pred, fake_secret.long().view(len(fake_secret_pred)))

            if hparams.use_weighted_squared_error:
                SE_w = G_w(images)
                rc_loss = mean_weighted_absolute_error(images, cen_images, SE_w, C=hparams.weight_budget, lambd=hparams.lambd)
            else:
                rc_loss = mean_absolute_error(images, cen_images)
        ...
        secret_pred = Ds(cen_images.detach())
    ...
    if hparams.use_weighted_squared_error:
        return images, cen_images, SE_w
    else:
        return images, cen_images

def validate(G, G_w, Ds_fix, Du_fix, validationloader, hparams):
    ...
    for i_batch, batch in tqdm.tqdm(enumerate(validationloader, 0)):
        ...
        secret_pred_fix = Ds_fix(cen_images)
        utility_pred_fix = Du_fix(cen_images)
    ...
    if not G_w is None:
        se_w = G_w(images)
        return (secret_acc, utility_acc), (images, cen_images, se_w)
    else:
        return (secret_acc, utility_acc), (images, cen_images)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_epochs", type=int, default=720)
